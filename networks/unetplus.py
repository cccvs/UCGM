...
class SongUNet(torch.nn.Module):
    def __init__(
        self,
        input_size=32,  # Image resolution at input/output.
        ...
        self.map_label = (
            Linear(in_features=num_classes, out_features=noise_channels, **init)
            if num_classes >= 2
            else None
        )
        self.map_augment = (
    ...
    def forward(self, x, t, y=None, augment_labels=None):
        # Mapping.
        t = (t * 2.68 - 1.59).flatten()  # EDM adapted to UCGM
...
class DhariwalUNet(torch.nn.Module):
    def __init__(
        self,
        input_size,  # Image resolution at input/output.
    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        ...
        class_labels = (
            ...
            if self.num_classes == 0
                ...
                torch.zeros([1, self.num_classes], device=x.device)
                if class_labels is None
                else class_labels.to(torch.float32).reshape(-1, self.num_classes)
            )
        )
        dtype = (
            torch.float16
...
class EDMPrecond(torch.nn.Module):
    def __init__(
        self,
        input_size=32,  # Image resolution.
