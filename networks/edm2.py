...
class UNet(torch.nn.Module):
    def __init__(
        ...
        **block_kwargs,  # Arguments for Block.
        ...
        self.emb_label = (
            MPConv(num_classes, cemb, kernel=[]) if num_classes != 0 else None
        )
    ...
    def forward(self, x, t, y, uncond=False):
        noise_labels = (t * 2.68 - 1.59).flatten()  # EDM2 adapted to UCGM

        # Embedding.
        emb = self.emb_noise(self.emb_fourier(noise_labels))
        if (self.emb_label is not None) and (uncond is not True):
            class_labels = F.one_hot(y, num_classes=self.num_classes)
            emb = mp_sum(
                emb,
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale, cfg_interval=None):
        ...
        Note that we follow edm impl for cfg here.
        """
        ...
        if t[0] >= cfg_interval[0] and t[0] <= cfg_interval[1]:
            ...
            model_out_lhalf = self.forward(
                combined[: len(x) // 2], t[: len(x) // 2], y[: len(x) // 2]
            )
            model_out_rhalf = self.forward(
                combined[len(x) // 2 :], t[len(x) // 2 :], y[len(x) // 2 :], True
            )
            model_out = torch.cat((model_out_lhalf, model_out_rhalf), dim=0)
...
def EDM2_XS(**kwargs):
    return UNet(model_channels=128, **kwargs)


def EDM2_S(**kwargs):
    return UNet(model_channels=192, **kwargs)


def EDM2_M(**kwargs):
    return UNet(model_channels=256, **kwargs)


def EDM2_L(**kwargs):
    return UNet(model_channels=320, **kwargs)


def EDM2_XL(**kwargs):
    return UNet(model_channels=384, **kwargs)


def EDM2_XXL(**kwargs):
    return UNet(model_channels=448, **kwargs)
